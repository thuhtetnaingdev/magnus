...
const AiderParametersSchema = z.object({
  instruction: z
    ...
    .describe(
      "The instruction for aider to execute (what code changes to make)"
    ),
  files: z
  ...
  model: z
    ...
    .optional()
    .describe(
      "OpenAI-compatible model to use (defaults to OPENAI_MODEL from env)"
    ),
});

export type AiderParameters = z.infer<typeof AiderParametersSchema>;

export interface AiderTool
  extends ToolDefinition<typeof AiderParametersSchema> {
  name: "aider";
}
    ...
    try {
      ...
      aiderProcess.on("close", (code) => {
        if (code === 0) {
          ...
          reject(
            new Error(
              `Aider process exited with code ${code}: ${stderr || stdout}`
            )
          );
        }
      });
