...
const TreeParametersSchema = z.object({
  path: z
    ...
    .optional()
    .default(".")
    .describe("The directory to generate tree structure for (defaults to current working directory)"),
  maxDepth: z
    .number()
    .int()
    .min(1)
...
export const treeTool = createTool({
  name: "tree" as const,
  description: "Generate a tree structure showing the project directory hierarchy for better project overview and file understandability",
          ...
          try {
            ...
            if (stats.isDirectory()) {
              ...
              const relevantExtensions = [
                ...
                ".toml", ".toml", ".lock", ".env", ".gitignore", ".dockerignore"
              ];
              ...
              if (hasRelevantExtension || ["package.json", "tsconfig.json", "README.md"].includes(item)) {
                tree.push({
                  name: item,
                  type: "file"
                });
